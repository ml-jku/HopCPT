...
class EpsilonContextGen:
    ...
    def __init__(self, mode, ts_ids: List[str] = None):
        ...
        calc = None
        if mode == 'uni':
            calc = self._ctx_univariate_and_eps if self._use_eps else self._ctx_univariate
        elif mode == 'uni_past_multi_step':
            calc = self._ctx_uni_past_multi_current_and_eps if self._use_eps else self._ctx_uni_past_multi_current
        elif mode == 'multi':
            calc = self._ctx_multivariant_and_eps if self._use_eps else self._ctx_multivariant
        else:
            raise ValueError(f"Unsupported mode: {self._full_mode}")

        if self._use_y_hat:
            calc_1 = lambda **kwargs: self.cat_y_hat(get_ctx=calc, **kwargs)
        else:
            calc_1 = calc
    ...
    def calc(self, X_past, Y_past, eps_past, X_step, Y_hat_step, ts_id_enc):
        """
    ...
    def calc_single(self, X_past, Y_past, eps_past, X_step, Y_hat_step, ts_id_enc):
        """
    ...
    def calib_data_to_ctx(self, calib_data: PICalibData, Y_hat, past_window, use_pre_calib_eps_for_calib,
        ...
        context_data = self.calc(X_past=X_past_windowed, Y_past=Y_past_windowed, eps_past=eps_past_windowed,
                                 X_step=X_step, Y_hat_step=Y_hat_step,
    ...
    def context_size(self, no_of_x_features, past_window_len, y_features=1):
    ...
    @staticmethod
    def _ctx_univariate_and_eps(Y_past, eps_past, **kwargs):
        return torch.cat((Y_past.view(Y_past.shape[0], -1), eps_past.view(eps_past.shape[0], -1)), dim=1)
    ...
    @staticmethod
    def _ctx_multivariant_and_eps(X_past, Y_past, X_step, eps_past, **kwargs):
        return torch.cat((X_past.reshape(X_past.shape[0], -1), Y_past.view(Y_past.shape[0], -1),
                          eps_past.view(eps_past.shape[0], -1), X_step.view(X_step.shape[0], -1)), dim=1)
    ...
    @staticmethod
    def _ctx_uni_past_multi_current_and_eps(Y_past, X_step, eps_past, **kwargs):
        return torch.cat((Y_past.view(Y_past.shape[0], -1), eps_past.view(eps_past.shape[0], -1),
                          X_step.view(X_step.shape[0], -1)), dim=1)
    @staticmethod
